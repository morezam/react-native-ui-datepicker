...
const Years = () => {
  ...
  const generateCells = (0, _react.useCallback)(() => {
    ...
    const column = years.map(year => {
      ...
      const textStyle = _reactNative.StyleSheet.flatten([styles.year_label, isActivated && styles.active_year_label, isSelected && styles.selected_year_label, isDisabled && styles.disabled_label, fontFamily ? {
        fontFamily
      } : null]);
