...
import { getParsedDate, getMonthDays, getDaysInMonth, areDatesOnSameDay, isDateBetween, getDate } from '../utils';
...
const Days = () => {
  ...
  const containerStyle = useMemo(() => [style.daysContainer, styles === null || styles === void 0 ? void 0 : styles.days], [style.daysContainer, styles === null || styles === void 0 ? void 0 : styles.days]);
  const daysGrid = useMemo(() => {
    ...
    return getMonthDays(currentDate, showOutsideDays, minDate, maxDate, firstDayOfWeek, enabledDates, disabledDates, prevMonthDays, prevMonthOffset, daysInCurrentMonth, daysInNextMonth, numerals).map((day, index) => {
      if (!day) return null;
      ...
      if (mode === 'range') {
        ...
        if (isFirstDayOfMonth && selectedEndDay || isLastDayOfMonth && selectedStartDay || startDate && endDate && dayjs(startDate).format('DDMMYYYY') === dayjs(endDate).format('DDMMYYYY')) {
          inRange = false;
        }
